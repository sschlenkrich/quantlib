...
namespace QuantLib {
	...
	OptionletStripper1::OptionletStripper1(
		...
		volQuotes_(nOptionletTenors_, std::vector<shared_ptr<SimpleQuote> >(nStrikes_)), floatingSwitchStrike_(switchStrike==Null<Rate>() ? true : false),
		capFlooMatrixNotInitialized_(true), switchStrike_(switchStrike), accuracy_(accuracy), maxIter_(maxIter), useNormalVols_(useNormalVols) {
	...
	void OptionletStripper1::performCalculations() const {
		...
		shared_ptr<BlackCapFloorEngine> dummy(new BlackCapFloorEngine(iborIndex_->forwardingTermStructure(),0.20, dc)); // discounting does not matter here
		for (Size i=0; i<nOptionletTenors_; ++i) {
			CapFloor temp = MakeCapFloor(CapFloor::Cap, capFloorLengths_[i], iborIndex_, 0.04, 0*Days).withPricingEngine(dummy); // dummy strike
		...
		const Handle<YieldTermStructure>& discountCurve = discount_.empty() ? iborIndex_->forwardingTermStructure() : discount_;

		const std::vector<Rate>& strikes = termVolSurface_->strikes();
		// initialize CapFloorMatrix
		if (capFlooMatrixNotInitialized_) {
			for (Size i=0; i<nOptionletTenors_; ++i) capFloors_[i].resize(nStrikes_);
			...
			for (Size j=0; j<nStrikes_; ++j) {
				...
				for (Size i=0; i<nOptionletTenors_; ++i) {
					...
					shared_ptr<BlackCapFloorEngine> engine(new BlackCapFloorEngine(discountCurve,Handle<Quote>(volQuotes_[i][j]),dc));
					if (useNormalVols_) engine = shared_ptr<BlackCapFloorEngine>(new BachelierBlackCapFloorEngine(discountCurve,Handle<Quote>(volQuotes_[i][j]),dc) );
					capFloors_[i][j] = MakeCapFloor(capFloorType,capFloorLengths_[i], iborIndex_,strikes[j], 0*Days).withPricingEngine(engine);
				}
			}
		...
		for (Size j=0; j<nStrikes_; ++j) {
			...
			for (Size i=0; i<nOptionletTenors_; ++i) {
				...
				try {
					// switch here for Normal Vols
					if (useNormalVols_) {
						optionletStDevs_[i][j] = bachelierBlackFormulaImpliedVol(optionletType,strikes[j],atmOptionletRate_[i],optionletTimes_[i],optionletPrices_[i][j],optionletAnnuity) * std::sqrt(optionletTimes_[i]);
					} else {
						optionletStDevs_[i][j] = blackFormulaImpliedStdDev(optionletType,strikes[j],atmOptionletRate_[i],optionletPrices_[i][j],optionletAnnuity, 0.0,optionletStDevs_[i][j],accuracy_, maxIter_);
					}
					...
					//	"\n error:   " << e.what());
					// we don't want the bootstrapping to fail here
