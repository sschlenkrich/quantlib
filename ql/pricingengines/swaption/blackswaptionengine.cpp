...
namespace QuantLib {
    ...
    void BachelierBlackSwaptionEngine::calculate() const {
        static const Spread basisPoint = 1.0e-4;
        ...
        // swap.iborIndex() might be using a different forwarding curve
        swap.setPricingEngine(boost::shared_ptr<PricingEngine>(new
            DiscountingSwapEngine(termStructure(), false)));
        Rate atmForward = swap.fairRate();
        ...
        // the swap length calculation might be improved using the value date
        // of the exercise date
        Time swapLength =  volatility()->swapLength(exerciseDate,
                                                   arguments_.floatingPayDates.back());
        results_.additionalResults["swapLength"] = swapLength;

        Real variance = volatility()->blackVariance(exerciseDate,
                                                   swapLength,
                                                   strike);
        Real stdDev = std::sqrt(variance);
        results_.additionalResults["stdDev"] = stdDev;
        Option::Type w = (arguments_.type==VanillaSwap::Payer) ?
                                                Option::Call : Option::Put;
		// results_.value = blackFormula(w, strike, atmForward, stdDev, annuity, displacement_);
		results_.value = bachelierBlackFormula(w, strike, atmForward, stdDev, annuity);

        Time sqrtExerciseTime = std::sqrt(volatility()->timeFromReference(exerciseDate));
        //results_.additionalResults["vega"] = std::sqrt(exerciseTime) * blackFormulaStdDevDerivative(strike, atmForward, stdDev, annuity,displacement_);
		Real eps = 1.0e-4;
		results_.additionalResults["vega"] = (
			bachelierBlackFormula(w, strike, atmForward, stdDev+eps*sqrtExerciseTime, annuity) -
			bachelierBlackFormula(w, strike, atmForward, stdDev-eps*sqrtExerciseTime, annuity) ) / 2.0 / eps;
    }
